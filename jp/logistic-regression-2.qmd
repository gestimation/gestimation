...
::: voice-father
お父さん「操作を加えても構造が維持されるっていうこと。どんなに面を回しても、ルービックキューブは解くと絵の向きが揃うように作られてる。データを解析するとき、人工的に変換を加えることがあるよね。たとえば死亡じゃなくて生存をアウトカムにしたり、LDLコレステロールの単位をmg/dLからmmol/Lに変えたり。これは共変量$X_i$のスケール変換のことだよ」
::: 
