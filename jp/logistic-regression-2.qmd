...
::: voice-father
お父さん「それはリスク差のモデルだね。このモデルが悪いわけじゃないんだけど、ロジット関数が選ばれた理由はちゃんとある。データを解析するとき、人工的に変換を加えることがあるよね。たとえば死亡じゃなくて生存をアウトカムにしたり、LDLコレステロールの単位をmg/dLからmmol/Lに変えたり。これは共変量$X_i$のスケール変換のことだよ」
::: 
...
::: voice-father
お父さん「この"死亡と生存を入れ替える"という操作が群作用のこと。ここで問題にしているのはリンク関数$g$だったよね。確率$\pi$を、リンク関数で別のスケール$g(\pi)$に変換したとする。このとき、死亡と生存を入れ替えても、値が反転するだけで構造が維持されるには$g$はどんな性質を持つべき？」
:::
