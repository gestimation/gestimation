...
::: voice-daughter
私「で、それが`family`、`link`、`y ~ x1 + x2`なの？」
::: 

::: voice-father
お父さん「そう。`glm()`は、確率分布+リンク+回帰係数×共変量を指定してるだけなんだ」
::: 
...
::::::::::::::::::::::::::::::::: dialogue
::: voice-father
お父さん「最初の列はすべて1でしょ。これは回帰直線でいう切片のこと。次の列以降はx1とx2の値だよね。
ちょっとマニアックだけど、Rの内部では、`model.matrix()`っていう関数で、デザイン行列Xを作っている。
こうしておくと、たとえばx1が文字列だったときに計算できたり、回帰係数との掛け算が、行列計算になって都合がいい」
::: 
...
::: voice-father
お父さん「そう。`family`は日本語では分布族の意味だね。前にいったみたいに、データの型にあわせて確率分布を選ぶ」
::: 
::::::::::::::::::::::::::::::::: 
...
::::::::::::::::::::::::::::::::: dialogue
::: voice-father
お父さん「そう。`family`は日本語では分布族の意味だね。前にいったみたいに、データの型にあわせて確率分布を選ぶ。ただし、一般化線型モデルではデフォルト仕様みたいなものがあって、`family`を指定すると以下の3つが一度に決まるっていう特徴がある」
::: 
...
::: voice-father
お父さん「リンク関数は**平均をどう変換して回帰係数×共変量にあわせるか**だよ。`family = binomial()`は、デフォルトでロジット関数というリンク関数を使っている。これがロジスティック回帰だよ」
::: 

::: voice-daughter
私「そっか、よかった。うん、だいたいわかったから今日はもういい」
::: 
::::::::::::::::::::::::::::::::: 
